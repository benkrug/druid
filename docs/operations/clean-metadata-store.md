...
You can configure the Overlord to delete indexer task log metadata and the indexer task logs from local disk or from cloud storage.  The cleanup includes the druid_tasks and druid_tasklogs tables in the metadata database, and the task logs in deep storage.  (Note that druid_tasklogs is not used and will already be empty, unless the druid version is older.) 
